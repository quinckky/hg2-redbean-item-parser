...
def connect(server: Literal['asia', 'china', 'original', 'beta', 'jporiginal'], driver) -> None:
    ...
    element = driver.find_element(
        By.CSS_SELECTOR, '.items-per-page > input:nth-child(1)')
    element.send_keys('00')

    time.sleep(1)


def _open_info(id_: int, driver) -> bool:
    try:
...
def get_main_info(driver) -> dict:
    ...
    try:
        ...
        main_info['Damage Type'] = DAMAGE_TYPE_NAMES[element.get_attribute(
            'src').split('/')[-1][:-4]]
...
def get_properties(driver) -> dict:
    ...
    elements = driver.find_elements(
        By.CSS_SELECTOR, 'span.detail-secondary-title')
    names = [element.text[:-1] for element in elements]

    elements = driver.find_elements(
        By.CSS_SELECTOR, 'span.detail-secondary-prop')
    values = [element.text for element in elements]

    return {PROPERTY_NAMES.get(name, name): WEAPON_TYPE_NAMES.get(
        value, value if value != '特殊-弓' or server != 'asia' else 'Bow') 
        for name, value in zip(names, values)}


def get_skills(driver) -> list:
    elements = driver.find_elements(
        By.CSS_SELECTOR, '.detail-skill-damage-type')
    damage_types = [element.get_attribute('src').split(
        '/')[-1][:-4] for element in elements]
    damage_types = [DAMAGE_TYPE_NAMES[damage_type]
    ...
    return [(damage_type, title, description.strip()) 
            for damage_type, title, description 
            in zip(damage_types, titles, descriptions)]


def get_item(id_: int, driver) -> tuple[dict, dict, list] | None:
    if not _open_info(id_, driver):
...
if __name__ == '__main__':
    driver = Firefox(service=FirefoxService(
        GeckoDriverManager().install(), log_output='driver.log'))
    connect('asia', driver)
